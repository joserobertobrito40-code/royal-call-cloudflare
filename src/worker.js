export class Room {
  ...
  async fetch(request) {
    ...
    this.clients.set(id, {
      ws: server,
      role: authorizedRole,
      approved: authorizedRole === "teacher"
    });

    server.addEventListener(
      "message",
      event => this.onMessage(id, event.data)
    );
  ...
  onClose(id) {
    ...
    if (departing?.role === "student") {
      this.sendToRole("teacher", {
        type: "student-left",
        studentId: id
      });

    } else if (departing?.role === "teacher") {
...
async function turnCredentials(env, request) {
  ...
  if (
    ...
    return Response.json(
      {
        error: "TURN secrets are not configured"
      },
      {
        status: 500,
        headers: cors(origin)
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/turn") {
      return turnCredentials(env, request);
    }
